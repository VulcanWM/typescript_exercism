export function clean (string: string): string | never {
  if ((/[A-Za-z]/u).test(string)) throw new Error('Letters not permitted');
  if ((/[@:!]/u).test(string)) throw new Error('Punctuations not permitted');
  
  let result = string.replace(/[^\d]/gu, '');
